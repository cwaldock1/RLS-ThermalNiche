...
All_Gams_Preds <- left_join(All_Gams_Preds, ThermalNicheData %>% dplyr::select(SpeciesName, ConfidenceCombined, T_Gam_edf, T_Lower_0.1, T_Upper_Obs, T_Lower_0.25, 
                                                                               T_Upper_0.25, T_Lower_0.5, T_Upper_0.5, T_SD_Upper_OBS, T_SD_Lower_OBS))
...
All_Gams_Preds2 <- All_Gams_Preds %>% group_by(SpeciesName) %>% nest() %>% mutate(ScaledAbundance = purrr::map(data, ~as.numeric(.$Abundance / max(.$Abundance))), 
                                                                                  CentredTemp     = purrr::map(data, ~ 
                                                                                                                 ...
                                                                                                                 (mean(c(.$T_SD_Upper_OBS[1], .$T_SD_Lower_OBS[1]))))
                                                                                  ) %>% 
  unnest()
...
pdf('figures_new/SamplingLimits/ThermalDistributionsFigure_allConf.pdf', height = 4, width = 8)
...
pdf('figures_new/SamplingLimits/ThermalDistributionsFigure_allConf_V2.pdf', height = 4, width = 8)
ggplot(All_Gams_Preds2 %>% filter(AbundanceDistributionCategory != 'No trend')) + 
  geom_line(aes(x = CentredTemp, y = ScaledAbundance, col = ThermalGuild, group = SpeciesName, alpha = as.factor(as.numeric(ConfidenceCombined)))) + 
  facet_grid(ThermalGuild~AbundanceDistributionCategory, scales = 'free_x') + 
  ylim(c(0,1)) + xlim(c(-4, 4)) + 
...
ThermalNicheData$T_SD_Upper_OBS <- abs(ThermalNicheData$T_Upper_Obs - ThermalNicheData$Topt) / 1.96
ThermalNicheData$T_SD_Lower_OBS <- abs(ThermalNicheData$T_Lower_Obs - ThermalNicheData$Topt) / 1.96

# Bind in thermal niche parameters to standardised species temperature and abundance ranges. 
RLS_All_JAGS <- RLS_All %>% left_join(., ThermalNicheData %>% dplyr::select(SpeciesName, Topt, T_SD_Upper_OBS, T_SD_Lower_OBS, ThermalGuild))
...
RLS_All_JAGS_scales <- RLS_All_JAGS %>% 
  ...
  mutate(Species_MeanTemp_CoralWatch  = purrr::map(data, ~ (.$MeanTemp_CoralWatch - unique(.$Topt))/mean(c(.$T_SD_Upper_OBS , .$T_SD_Lower_OBS))), 
...
RLS_All_JAGS$T_SD_2                          <- RLS_All_JAGS$T_SD_Upper_OBS
RLS_All_JAGS$T_SD                            <- RLS_All_JAGS$T_SD_Lower_OBS
...
dir.create('data_derived/jagsmodels/SamplingLimits')
...
TropJAGS_0.99_ABUN <- RunJAGSModels(modeldata = TropData_JAGS %>% filter(AbundanceAdult40 > 0),
                                    ...
                                    n.burnin   = 2500,
                                    n.iter     = 10000)
saveRDS(TropJAGS_0.99_ABUN, 'data_derived/jagsmodels/SamplingLimits/TropJAGS_0.99_ABUN.rds')
TropJAGS_0.99_ABUN <- readRDS('data_derived/jagsmodels/SamplingLimits/TropJAGS_0.99_ABUN.rds')
...
MCMCtrace(TropJAGS_0.99_ABUN$models[[1]], 
          params = c('T_Opt', 'MaxAbun', 'T_Breadth', 'Skew'),
          ISB = FALSE,
          pdf = FALSE)

MCMCplot(TropJAGS_0.99_ABUN$models[[1]], 
         params = c('T_Opt', 'MaxAbun', 'T_Breadth', 'Skew'),
         ref_ovl = TRUE)
...
TempJAGS_0.99_ABUN <- RunJAGSModels(modeldata = TempData_JAGS %>% filter(AbundanceAdult40 > 0),
                                    ...
                                    n.burnin   = 2500,
                                    n.iter     = 10000)
saveRDS(TempJAGS_0.99_ABUN, 'data_derived/jagsmodels/SamplingLimits/TempJAGS_0.99_ABUN.rds')
TempJAGS_0.99_ABUN <- readRDS('data_derived/jagsmodels/SamplingLimits/TempJAGS_0.99_ABUN.rds')
...
MCMCtrace(TempJAGS_0.99_ABUN$models[[1]], 
          params = c('T_Opt', 'MaxAbun', 'T_Breadth', 'Skew'),
          ISB = FALSE,
          pdf = FALSE)

MCMCplot(TempJAGS_0.99_ABUN$models[[1]], 
         params = c('T_Opt', 'MaxAbun', 'T_Breadth', 'Skew'),
         ref_ovl = TRUE)
...
TropJAGS_0.99_ABUN_AGG <- RunJAGSModels(modeldata = TropData_JAGS %>% filter(AbundanceAdult40 > 0),
                                        ...
                                        n.burnin   = 2500,
                                        n.iter     = 10000)
saveRDS(TropJAGS_0.99_ABUN_AGG, 'data_derived/jagsmodels/SamplingLimits/TropJAGS_0.99_ABUN_AGG.rds')
TropJAGS_0.99_ABUN_AGG <- readRDS('data_derived/jagsmodels/SamplingLimits/TropJAGS_0.99_ABUN_AGG.rds')
...
TempJAGS_0.99_ABUN_AGG <- RunJAGSModels(modeldata = TempData_JAGS %>% filter(AbundanceAdult40 > 0),
                                        ...
                                        n.burnin   = 2500,
                                        n.iter     = 10000)
saveRDS(TempJAGS_0.99_ABUN_AGG, 'data_derived/jagsmodels/SamplingLimits/TempJAGS_0.99_ABUN_AGG.rds')
TempJAGS_0.99_ABUN_AGG <- readRDS('data_derived/jagsmodels/SamplingLimits/TempJAGS_0.99_ABUN_AGG.rds')
...
pdf('figures_new/SamplingLimits/Abundance_TPC_ThermalGuild.pdf', width = 8, height = 4, useDingbats = FALSE, bg = 'transparent')
ggplot() + 
...
pdf('figures_new/SamplingLimits/Abundance_TPC_ThermalGuild_temperate.pdf', width = 4, height = 4, useDingbats = FALSE, bg = 'transparent')
ggplot() + 
  # Minor data points
...
pdf('figures_new/SamplingLimits/Abundance_TPC_ThermalGuild_tropical.pdf', width = 4, height = 4, useDingbats = FALSE, bg = 'transparent')
ggplot() + 
  # Minor data points
