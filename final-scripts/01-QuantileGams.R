...
FitModels_27092015 <- function(Species1, k = 4, q = 0.8, NRUNS = 1, Folder = NULL){
  ...
  if(length(which(apply(apply(Species1, 2, is.na), 2, sum) > 1)) != 0){Species1 <- Species1[,-which(apply(apply(Species1, 2, is.na), 2, sum) > 1)]}
...
# MODELLING QGAMS WITH AUTOCORRELATION DATA SAVED ----
# Bind in latitudes and longitues ----
RLS_SiteLocations <- read.csv('data_upload/RLS_SiteLocations.csv')
RLS_20 <- left_join(RLS_20, RLS_SiteLocations)
Species1 <- RLS_20 %>% filter(SpeciesName == unique(.$SpeciesName)[351]) # %>% ConstrainAbsences(.)

# Create new function that outputs data that can be tested for spatial auto-correlation
FitModels_20171109 <- function(Species1, k = 4, q = 0.8, NRUNS = 1, Folder = NULL, changeWD = F){
...
Species1$Depth_Site                     <- as.numeric(scale(Species1$Depth_Site))
Species1$NEOLI[is.na(Species1$NEOLI)]   <- 0
#Species1$pos <- numFactor(Species1$SiteLong, Species1$SiteLat)
#Species1$group <- factor(rep(1, nrow(Species1)))
...
### --- Create output that I can check spatial auto-correaltion with 
if(NrowPresences < NrowAbsences){
  
  Spp_Autocorr <- data.frame(SpeciesName = Species1_Subset$SpeciesName, 
             SiteCode = Species1_Subset$SiteCode,
             Latitude = Species1_Subset$SiteLat, 
             Longitude = Species1_Subset$SiteLong, 
             qgam_resid = resid(Species1_GAM), 
             AbundanceAdult40_log = Species1_Subset$AbundanceAdult40_log, 
             AbundanceAdult40  = Species1_Subset$AbundanceAdult40)}else{
  Spp_Autocorr <-  data.frame(SpeciesName = Species1$SpeciesName, 
               SiteCode = Species1$SiteCode,
               Latitude = Species1$SiteLat, 
               Longitude = Species1$SiteLong, 
               qgam_resid = resid(Species1_GAM), 
               AbundanceAdult40_log = Species1$AbundanceAdult40_log, 
               AbundanceAdult40  = Species1$AbundanceAdult40)
             }


OutputData <- list(model = Species1_GAM, predictions = PredData, autocorrelation_data = Spp_Autocorr)
...
saveRDS(OutputData, file = paste(paste0(Folder, '/'), gsub(' ' , '_', unique(Species1$SpeciesName)), gsub('-','_',Sys.Date()), 'q_is', gsub('0.','',q),'k_is',k,'.rds', sep = '_'))
  ...
  # Make sure the folder here is the full file path (i.e., if saving on an external device)
  saveRDS(OutputData, file = paste(paste0(Folder, '/'), gsub(' ' , '_', unique(Species1$SpeciesName)), gsub('-','_',Sys.Date()), 'q_is', gsub('0.','',q),'k_is',k,'.rds', sep = '_'))
}
rm(Species1_GAM)
...
# Testing function 
TestSpecies <- RLS_20 %>% filter(SpeciesName == unique(.$SpeciesName)[231]) # %>% ConstrainAbsences(.)
TestSpecies <- RLS_20 %>% filter(SpeciesName == Bottom10[9])# %>% ConstrainAbsences(.)
FitModels_20171109(Species1 = TestSpecies, Folder = 'TEST')
FitModels_20171109(Species1 = TestSpecies, Folder = '/Volumes/Untitled/Raster_files/qgamOutputs/', changeWD = T)
...
# Runs function in parallel and saves qgams into a new folder. 
# Here we just test a single run to get the autocorrelation temrs 
a <- Sys.time()
cl <- makeCluster(4)
registerDoParallel(cl)
ModelOutputs <- foreach(i=1:length(unique(RLS_20$SpeciesName)), 
                        .packages=c('tidyr', 'dplyr', 'glmmTMB', 'qgam')) %dopar% {
                          tryCatch(FitModels_20171109(RLS_20[which(RLS_20$SpeciesName == unique(RLS_20$SpeciesName)[i]),], 
                                                      Folder = '/Volumes/Untitled/Raster_files/qgamOutputs/', NRUNS = 1, changeWD = T), error = function(e) NA)
                        }
stopCluster(cl)
b <- Sys.time() - a # Takes ~5 hours to run. 
b
...
# Combine output and save as a file. This is the parameters for the Topt. 
Quantile_Parameters <- do.call(rbind, ModelOutputs)
saveRDS(Quantile_Parameters, file = 'data_derived/qGamModelOutputs-2019-11-09_AUTOCORRELATION.rds')
Quantile_Parameters <- readRDS(file = 'data_derived/qGamModelOutputs-2019-11-09_AUTOCORRELATION.rds')

# Read in models and residuals ----
QuantileModels <- list.files('/Volumes/Untitled/Raster_files/qgamOutputs/',full.names=F)

qgamFiles_ConfAll <- paste('/Volumes/Untitled/Raster_files/qgamOutputs/', QuantileModels, sep = '')

M1 <- readRDS(file = qgamFiles_ConfAll[1])$autocorrelation_data

M1$model$residuals
M1$model$fitted.values
M1$model$linear.predictors

# Create list of model outputs for all species. 
All_autocorrelation_data <- list()
for(i in 1:length(qgamFiles_ConfAll)){
  print(i)
  All_autocorrelation_data[[i]] <- readRDS(file = qgamFiles_ConfAll[i])$autocorrelation_data
}

#saveRDS(All_autocorrelation_data, 'data_derived/All_autocorrelation_data.rds')
All_autocorrelation_data <- readRDS(file = 'data_derived/All_autocorrelation_data.rds')

# Mantel tests 
SpatialAutocorrelation <- list()
for(i in 1:length(All_autocorrelation_data)){
  print(i)
  spatial_dist <- dist(All_autocorrelation_data[[i]] %>% filter(AbundanceAdult40 > 0) %>%  .[,c('Longitude', 'Latitude')])
  resid_dist   <- dist(All_autocorrelation_data[[i]] %>% filter(AbundanceAdult40 > 0) %>% .$qgam_resid)
  SpatialAutocorrelation[[i]] <- ade4::mantel.rtest(spatial_dist, resid_dist, 999)
}

# Most show non-significant spatial autocorrelation 
table(melt(lapply(SpatialAutocorrelation, function(x) x$pvalue))[,1] > 0.05)
table(melt(lapply(SpatialAutocorrelation, function(x) x$pvalue))[,1] > 0.01)
228/length(SpatialAutocorrelation)
139/length(SpatialAutocorrelation)

# Average correalation is very low
mean(abs(melt(lapply(SpatialAutocorrelation, function(x) x$obs))[,1]))
sd(abs(melt(lapply(SpatialAutocorrelation, function(x) x$obs))[,1]))
summary(abs(melt(lapply(SpatialAutocorrelation, function(x) x$obs))[,1]))
hist(melt(lapply(SpatialAutocorrelation, function(x) x$obs))[,1])

# Testing with Morans test too - shows similar to above. 
SpatialAutocorrelation_moran <- list()
for(i in 1:length(All_autocorrelation_data)){
  print(i)
  spdat <- SpatialPointsDataFrame(All_autocorrelation_data[[i]] %>% filter(AbundanceAdult40 > 0) %>%  .[,c('Longitude', 'Latitude')], 
                                  All_autocorrelation_data[[i]] %>% filter(AbundanceAdult40 > 0))
  lstw  <- nb2listw(knn2nb(knearneigh(spdat, k = 10)))
  SpatialAutocorrelation_moran[[i]] <- spdep::moran.test(All_autocorrelation_data[[i]] %>% filter(AbundanceAdult40 > 0) %>% .$qgam_resid, lstw) 
}

SpatialAutocorrelation_moran[[1]]$estimate

table(melt(lapply(SpatialAutocorrelation_moran, function(x) x$p.value))[,1] > 0.05)
table(melt(lapply(SpatialAutocorrelation_moran, function(x) x$p.value))[,1] > 0.01)

# Suggests weak but positive clustering. 
mean(abs(melt(lapply(SpatialAutocorrelation_moran, function(x) x$estimate[1]))[,1]))
summary(abs(melt(lapply(SpatialAutocorrelation_moran, function(x) x$estimate[1]))[,1]))
hist(melt(lapply(SpatialAutocorrelation_moran, function(x) x$estimate[1]))[,1])
